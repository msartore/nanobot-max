...
def _make_loop():
    ...
    with patch("nanobot.agent.loop.ContextBuilder"), \
        ...
        loop = AgentLoop(bus=bus, provider=provider, workspace=workspace, max_completion_checks=0)
    return loop, bus
...
class TestRestartCommand:
    ...
    async def test_status_reports_runtime_info(self):
        ...
        assert "Context: 20k/64k (31% of input budget)" in response.content
    ...
    async def test_status_falls_back_to_last_usage_when_context_estimate_missing(self):
        ...
        assert "Context: 1k/64k (1% of input budget)" in response.content
        assert "Tasks: 0 active" in response.content
