...
@pytest.mark.asyncio
async def test_context_too_long_irreducible_terminates():
    """When context is already at minimum (2 messages: system + user) and still
    exceeds the context window, the runner must terminate rather than loop forever."""
    runner = _make_runner()
    call_count = {"n": 0}

    # Only 2 messages — hard-truncation cannot reduce this further
    initial_messages = [
        {"role": "user", "content": "x" * 1000},
    ]

    context_length_error = LLMResponse(
        content="Error: {'message': \"This endpoint's maximum context length is 262144 tokens. However, you requested about 338991 tokens (253995 of text input, 3076 of tool input, 81920 in the output).\", 'code': 400, 'metadata': {'provider_name': None}}",
        finish_reason="error",
        tool_calls=[],
        usage={},
    )
    ...
    result = await runner.run(AgentRunSpec(
        ...
        tools=_make_tools(),
        model="test-model",
        max_iterations=20,  # high limit — must not be reached
        max_tool_result_chars=16000,
        max_completion_checks=0,
    ))

    assert result.stop_reason == "context_overflow"
    assert "too large" in (result.final_content or "").lower()
    # Must terminate quickly, not spin through all 20 iterations
    assert call_count["n"] <= 3, (
        f"Expected early termination, but made {call_count['n']} LLM calls"
    )
